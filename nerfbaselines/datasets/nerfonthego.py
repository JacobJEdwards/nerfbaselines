...
def preprocess_nerfonthego_dataset(path, output):
    ...
    os.makedirs(os.path.join(output, f'images_8_multiexposure'), exist_ok=True)
    for image in os.listdir(os.path.join(output, 'images')):
        os.system(f"convert {os.path.join(output, 'images', image)} -resize {100 / scale}% -quality 100 "
                  f"{os.path.join(output, f'images_8_multiexposure', image)}")
...
def load_nerfonthego_dataset(path: str, split: str, **kwargs) -> UnloadedDataset:
    ...
    dataset = load_colmap_dataset(path, split=split, 
                                  images_path=f"images_8_multiexposure",
                                  **kwargs)
